...
async function sendTweet() {
  ...
  getInfo("/user-info", "GET", true).then((data) => {
...
function getTweetId(event) {
  ...
  getInfo("/tweets", "GET", true).then((data) => {
...
function updateTweet() {
  ...
  getInfo(`/api-update-tweet/${current_update_tweet_id}/${new_tweet_text}`, "PUT", false).then((data) => {
    console.log(data);
    if (data.status == 200) {
...
function addFollowing(event) {
  ...
  if (event.target.innerText == "Follow") {
    getInfo(`/follow-user/${following_id}`, "POST", false).then((data) => {
      console.log(data);
      console.log("hi");
      event.target.innerText = "Following";
    });
  } else {
    getInfo(`/unfollow-user/${following_id}`, "DELETE", false).then((data) => {
      console.log(data);
...
function getUserProfile(event) {
  ...
  getInfo(`/user-profile-info/${user_id}`, "GET", true).then((userInfo) => {
    console.log(userInfo);
    showUserProfile(userInfo);
  });

  getInfo(`/user-tweets/${user_id}`, "GET", true).then((userInfo) => {
    console.log(userInfo);
...
function showUserTweets(tweets, hideInput) {
  ...
  if (hideInput) {
    _one("#add-tweet").classList.add("hidden");
  }
...
function getTweets() {
  ...
  getInfo(`/tweets`, "GET", true).then((tweets) => {
    console.log(tweets);
    let searchResults = [];
